...
| `build-mark.cjs` | Generates `mark.svg`, `mark-white.svg`, `mark-ink.svg`, the transparent icon PNGs, the opaque `apple-touch-icon.png` (mark on PAPER, because iOS backs a transparent touch icon with black), the Chrome extension icon set under `docs/brand/extension/`, `docs/brand/preview/mark-alt.svg` and the review sheets; runs the alpha, opacity and maskable checks |
...
`docs/brand/extension/icon-16.png`, `icon-32.png`, `icon-48.png` and
`icon-128.png` are the sizes a Chrome MV3 manifest asks for (`icons` and
`action.default_icon`). They are generated by `build-mark.cjs` from the same
spec, transparent, with the mark filling the canvas edge to edge apart from the
spec's own 4-unit margin (no maskable inset, since Chrome does not mask them).
They are for the **extension project**, which lives outside this repository,
and are **not served by the web app**. Drop them into the extension's `icons/`
folder and point its manifest at them; the extension still ships SoundCloud's
cloud glyph as its listing icon until that happens.
